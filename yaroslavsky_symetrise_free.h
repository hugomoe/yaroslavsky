...
float *ligne_trans=malloc(2*dimax*sizeof(float)) ; //vecteur pour le deuxième shear
float *one_col_trans=malloc(2*dimax*sizeof(float)) ;//vecteur pour le premier shear
float *two_col_trans=malloc(2*dimax*sizeof(float)) ; //vecteur pour le troisième shear
float pi;
...
int i_sym,j_sym;
	for(int i=dimax-x;i<dimax+x;i++){
			for(int j=dimax-z;j<dimax+z;j++){
                i_sym = i-dimax+x/2;
                while(i_sym<0 || i_sym>x-1){i_sym = (i_sym<0) ? -1-i_sym : 2*x-1-i_sym;}
                j_sym = j-dimax+z/2;
                while(j_sym<0 || j_sym>z-1){j_sym = (j_sym<0) ? -1-j_sym : 2*z-1-j_sym;}
...
// au milieu à droite
     for (int i=x/2;i<x;i++){
		for (int j=0;j<z;j++){
		 	img_aux[dimax+3*x/2-(i+1)+(2*dimax)*(dimax/2+(dimax-z)/2+j+1)]=img[(i+j*x)*3+p] ;
		}
	}
// au milieu à gauche
     for (int i=0;i<x/2;i++){
		for (int j=0;j<z;j++){
		 	img_aux[dimax/2+(dimax-x)/2-(i+1)+(2*dimax)*(dimax/2+(dimax-z)/2+j)]=img[(i+j*x)*3+p] ;
		}
	}
// en haut au milieu
     for (int i=0;i<x;i++){
...
// en bas au milieu
 for (int i=0;i<x;i++){
		for (int j=z/2;j<z;j++){
		 	img_aux[dimax/2+(dimax-x)/2+i+(2*dimax)*(-j+dimax+z+z/2-1)]=img[(i+j*x)*3+p] ;
		}
	}
// en bas à gauche
 for (int i=0;i<x/2;i++){
		for (int j=z/2;j<z;j++){
		 	img_aux[dimax/2+(dimax-x)/2-i-1+(2*dimax)*(dimax+z+z/2-j-1)]=img[(i+j*x)*3+p] ;
		}
	}
...
// en bas à droite
 for (int i=x/2;i<x;i++){
		for (int j=z/2;j<z;j++){
		 	img_aux[dimax+3*x/2-i-1+(2*dimax)*(dimax+z+z/2-j-1)]=img[(i+j*x)*3+p] ;
		}
	}
...
// en haut à droite
 for (int i=x/2;i<x;i++){
		for (int j=0;j<z/2;j++){
		 	img_aux[dimax+3*x/2-i-1+(2*dimax)*(dimax/2+(dimax-z)/2-j)]=img[(i+j*x)*3+p] ;
		}
	}



// fin de "conditions aux bords"
*/
...
int trans_ligne(float *img,int w,int h,float *vect_trans,int l){
		...
		float *ligneimg=malloc(w*sizeof(float));
                float *lignerefftimg=malloc(w*sizeof(float));
		float *ligneimfftimg=malloc(w*sizeof(float));
...
int trans_colonne(float *img,int w,int h,float *vect_trans,int l){
		...
		float *imfftimg=malloc(w*h*sizeof(float));
		float *refftrans=malloc(w*h*sizeof(float));
                float *ligneimg=malloc(h*sizeof(float));
                float *lignerefftimg=malloc(h*sizeof(float));
		float *ligneimfftimg=malloc(h*sizeof(float));
